		...
		for {
			...
			if c == '<' {
				if l.r.Peek(1) == '/' {
					...
					if h := ToHash(parse.Copy(l.r.Lexeme()[mark+2:])); h == l.rawTag { // copy so that doesn't change the case of the underlying slice
						l.r.Rewind(mark)
						return l.r.Shift()
					}
					...
					for {
						...
						if c == '-' && l.r.Peek(1) == '-' && l.r.Peek(2) == '>' {
							...
							if h := ToHash(parse.Copy(l.r.Lexeme()[mark:])); h == Script { // copy so that doesn't change the case of the underlying slice
								if !isEnd {
									inScript = true
								} else {
									if !inScript {
...
func (l *Lexer) shiftXml(rawTag Hash) []byte {
	...
	for {
		...
		if c == '"' {
			...
			if h := ToHash(parse.Copy(l.r.Lexeme()[mark+2:])); h == rawTag { // copy so that doesn't change the case of the underlying slice
				break
			}
		} else if c == 0 {
			if l.r.Err() == nil {
